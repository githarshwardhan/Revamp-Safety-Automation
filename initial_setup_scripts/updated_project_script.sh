...
RESULT=$(mongosh --host localhost --port 27017 -u adminuser -p  saFeTyaPp2o24  --authenticationDatabase admin --quiet --eval "db.projects.find({ 'projectName': '$PROJECT_NAME' }).count()" $DB_NAME)
...
CHECK=$(mongosh --host localhost --port 27017 -u adminuser -p  saFeTyaPp2o24  --authenticationDatabase admin --quiet --eval "db.projects.find({ 'projectName': '$PROJECT_NAME' }).count()" clientadmindb)
...
cd  /opt/build/project-build/safety-revamp-${PROJECT_Directory_NAME}-build
rm -rf /opt/build/project-build/safety-revamp-${PROJECT_Directory_NAME}-build
git clone https://token:AhaQ2ovu_4AUe8d1MQPm@gitlab.valueaddsofttech.com/safetyrevamp/vsa-revamp-project.git -b $GIT_TAG
cd /opt/build/project-build/safety-revamp-${PROJECT_Directory_NAME}-build/vsa-revamp-project
latest_tag=`cat version.txt`
echo "#################### $latest_tag #########################"
 mkdir -p /opt/build/project-build/safety-revamp-${PROJECT_Directory_NAME}-build/vsa-revamp-project/uploads/project
 env
 . ~/.nvm/nvm.sh
  nvm use v18.12.1
  npm install
...
#Build the Docker image
        ...
        cd /opt/build/cksafetyrevamp/project/
        docker build -t $GIT_Project_BRANCH:$latest_tag .
        docker tag $GIT_Project_BRANCH:$latest_tag 882647124176.dkr.ecr.ap-south-1.amazonaws.com/revampsafety/pre_dev:$GIT_Project_BRANCH-$latest_tag
        docker push 882647124176.dkr.ecr.ap-south-1.amazonaws.com/revampsafety/pre_dev:$GIT_Project_BRANCH-$latest_tag
